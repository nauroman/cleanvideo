...
During HYPIR export, enhanced frames are cached as PNG files under `work/cache` using the selected video and HYPIR settings. If the app is stopped mid-export, starting the same export again reuses already enhanced frames instead of generating them again. SeedVR2 and FlashVSR are video-native engines, so their exports run as whole-video processes and do not expose partial frame export. FlashVSR export is chunked internally; if a run fails after completing one or more chunks, starting the same export again reuses valid completed chunks instead of rerendering them. After a finished H.264 export, temporary cache files are scheduled for automatic cleanup. The browser also stores the last selected server-side video id, active job id, and UI settings in `localStorage`.
...
FlashVSR's official pipeline needs enough frames for its chunk loop. Short synthetic clips with 5 frames fail inside the official pipeline; the preview helper creates a 21-frame still clip for FlashVSR.
