...
During export, enhanced frames are cached as PNG files under `work/cache` using the selected video and HYPIR settings. If the app is stopped mid-export, starting the same export again reuses already enhanced frames instead of generating them again. After a finished H.264 export, the temporary frame cache is scheduled for automatic cleanup. The browser also stores the last selected server-side video id and UI settings in `localStorage`.
