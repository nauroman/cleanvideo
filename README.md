...
During HYPIR export, enhanced frames are cached as PNG files under `work/cache` using the selected video and HYPIR settings. If the app is stopped mid-export, starting the same export again reuses already enhanced frames instead of generating them again. SeedVR2 is a video-native engine and does not expose partial frame export. FlashVSR `Tiny Long` export runs as a single streaming model session that keeps the model context across the video instead of splitting the source into independent 21-frame files. The older 21-frame chunked path remains only as a fallback for `Tiny` and `Full`, so `Save Partial` is still chunk-based for those fallback variants but is not available during `Tiny Long` streaming. After a finished H.264 export, temporary cache files are scheduled for automatic cleanup. The browser also stores the last selected server-side video id, active job id, and UI settings in `localStorage`.
...
FlashVSR's official pipeline needs enough frames for its temporal loop. CleanVideo pads `Tiny Long` streaming with cloned tail frames and trims the output back to the source frame count. The preview helper still creates a 21-frame still clip for FlashVSR previews.
